...
bool   path_node_exists_in_open_list(MapGraph *g, int x, int y); /* Returns true if the supplied coordinates matches a node in the open_list */
...
void   path_node_update(MapGraph *g, int nx, int ny); /* updates the heuristics of a neighbor node if it has already been added to the open_list */
void   path_open_list_add_node(MapGraph *g, int x, int y); /* Adds a new node to open_list given the parent index, and coordinates on the map. */
