...
/*---------------------- Initialize map data ---------------------------------------------------------------------------------*/
extern void
gr_map_init(MapData *m, int map_width, int map_height)
{
...
extern int
gr_map_count_tile(MapData *m, char tile)
{
	int count = 0;
...
extern void
gr_map_gen_carve_hall_horizontally(MapData *m, Hall *halls_array, int hall_index, int xstart, int xend)
{
...
extern void
gr_map_gen_carve_hall_vertically(MapData *m, Hall *halls_array, int hall_index, int ystart, int yend)
{
...
/*---------------------- Carve rooms -----------------------------------------------------------------------------------------*/
extern void
gr_map_gen_carve_room(MapData *m, Room *rooms, int rooms_added)
{
...
extern void
gr_map_gen_carve_hallways(MapData *m, Room *rooms, int rooms_added)
{
	int xa,xb,ya,yb;
...
/*---------------------- Generate doors on the edges of each room ------------------------------------------------------------*/
extern void
gr_map_gen_doors(MapData *m, Room *rooms, int number_of_rooms)
{
		...
		for (int j = 1; j < (m->map_width-1); j++) { /* x coord */
			...
			if ( m->terrain[i * m->map_width + j] == m->floor) { /* check all candidates for doors */
				...
				if ( ((top == m->hallway_vertical && bottom ==  m->floor) || (top == m->floor && bottom ==  m->hallway_vertical)) && left == m->wall && right == m->wall) { /* top and bottom are floor */
					...
					for (int k = 0; k < number_of_rooms; k++) { /* vertical doors */
...
extern void
gr_map_gen_style_dungeon(MapData *m, Component *c)
{
...
extern void
gr_map_gen_style_cave(MapData *m, Component *c)
{
		...
		for (int j = 0; j < m->map_width; j++) {
			if ( i == 0 || i == m->map_height || j == 0 || j == m->map_width) { /* surround the map with walls. */
				...
				if ( m->map_height > 4 && i > (m->map_height / 2) && i < (m->map_height / 2) + 2) { /* create empty strip in middle of map */
					...
					if (rand_tile > 4) {
						terrain_copy[ i * m->map_width + j ] = m->floor; /* everything  is random - wall or floor */
					} else {
						terrain_copy[ i * m->map_width + j ] = m->wall;
					}
				}
	...
	while (n < iterations) {
		for (int i = 2; i < m->map_height - 2; i++) {
			for (int j = 2; j < m->map_width - 2; j++) {
				...
				if ( terrain_copy[ i * m->map_width + j ] == m->wall && neighbor_walls >= 4) {
					m->terrain[ i * m->map_width + j] = m->wall;
				} else if ( terrain_copy[ i * m->map_width + j ] == m->floor && neighbor_walls >= 5) {
				   	m->terrain[ i * m->map_width + j] = m->wall;
				} else {
				  	m->terrain[ i * m->map_width + j] = m->floor;
				}
			}
		}
...
extern void
gr_map_gen_entrance_and_exit(MapData *m)
{
...
extern void
gr_map_gen_add_components(MapData *m, Component *c)
{
	for (int i = 0; i < m->map_height; i++) {
		for (int j = 0; j < m->map_width; j++) {
...
extern void
gr_map_flood_fill(MapData *m, int rand_x, int rand_y, char symbol)
{
...
extern void
gr_map_los_raycast(MapData *m, Component *c, int id, int range)
{
...
gr_map_get_random_tile(MapData *m, char tile)
{
